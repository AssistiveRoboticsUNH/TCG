...
# network layer information for CNN
aud_layer_elements = [-1, 16, 32, 128, len(CLASSES_A)]
...
class AudioCNNClassifier:
    """
    CNN classifier for audio inputs.
    Classes are defined in the perception.constants file as CLASSES_A.
    """
    def __init__(self, filename="", learning_rate=1e-5):
        ...
        # specific graph and session needed to run two CNNs in same GPU
        self.graph = tf.Graph()
        with self.graph.as_default():
            ...
            self.aud_ph = tf.placeholder("float",
                                         [self.__batch_size, None, aud_dtype["cmp_h"] *
                                          aud_dtype["cmp_w"] * aud_dtype["num_c"]],
            ...
            self.aud_y_ph = tf.placeholder("float", [None, len(CLASSES_A)],
                                           name="aud_y_placeholder")

            # Build Model Structure
            # initializes variables
            self.pred_aud_set = self.execute_aud_var_set()
            ...
            self.cross_entropy_aud = tf.nn.softmax_cross_entropy_with_logits(
                labels=self.aud_y_ph, logits=self.execute_aud())

            # optimize the network
            self.optimizer_aud = tf.train.AdamOptimizer(learning_rate=self._alpha).minimize(
    ...
    def classify_input_ros(self, num_frames, aud_data, verbose=False):
        """
        ...
        :param aud_data: (numpy array) an array that contains the audio data
        :param verbose: (bool) print additional information
        """
        prediction = self.sess.run(self.observe, feed_dict={
            self.seq_length_ph: [num_frames],
            self.aud_ph: aud_data
        })
        if verbose:
            print("Classification: " + CLASSES_A[int(prediction[0])])
    ...
    def aud_model(self, seq_length, aud_ph, variable_scope, var_aud):
        ...
        :param aud_ph: (placeholder) an array that contains the audio data
        :param variable_scope: (variable_scope) scope for the temporal data
