...
if __name__ == '__main__':
    ...
    for f in files_set:
        if 'failB' not in f:
            counter += 1
            f = f.replace('tfrecords', 'temp_info').replace('.tfrecord', '.txt')
            print("processing {}/{}: {}".format(counter, num_files, f))
            timing_dict = dict()
            cnn_helper.adjust_timing_dict_for_audio(timing_dict, f)
            seq_len = max(timing_dict.values()) + min(timing_dict.values())
